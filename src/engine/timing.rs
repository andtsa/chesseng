//! chess is a timed game, here we deal with that
use std::cmp::Ordering;
use std::time::Duration;
use std::time::Instant;

use anyhow::Result;

use crate::Engine;

/// one hundred years :)
const MAX_TIME: Duration = Duration::from_secs(60 * 60 * 24 * 365 * 100);

/// maximum time for thinking
const MAX_THINK: Duration = Duration::from_secs(5 * 60);

/// never exit instantly
const MIN_THINK: Duration = Duration::from_millis(2);

/// The duration before the search should end to allow for submitting the move.
pub const SUBMIT_DURATION: Duration = Duration::from_millis(5);

/// If the time-remaining difference between the players is greater than this,
/// maybe we should change our time management a bit.
const CONCERN_THRESHOLD: Duration = Duration::from_secs(15);

/// the fraction of each increment that we should use per move
const INC_FACTOR: (u32, u32) = (3, 4);

/// Returns the maximum instant that can be represented.
pub fn max_instant() -> Instant {
    Instant::now() + MAX_TIME
}

impl Engine {
    /// Set the time until which the engine should search.
    pub fn game_time_constraints(
        &mut self,
        white_time: Option<Duration>,
        black_time: Option<Duration>,
        white_increment: Option<Duration>,
        black_increment: Option<Duration>,
        moves_to_go: Option<u8>,
    ) -> Result<()> {
        let (our_time, our_inc, their_time, their_inc) = match self.board.chessboard.side_to_move()
        {
            chess::Color::White => (white_time, white_increment, black_time, black_increment),
            chess::Color::Black => (black_time, black_increment, white_time, white_increment),
        };

        let now = Instant::now();

        // estimate how long the game still has to go.
        // TODO: improve this estimate.
        let est_moves_left = moves_to_go.unwrap_or(50).max(1) as u32;

        // build the time budget for this move based on all the information we got
        let mut budget = Duration::ZERO;

        if let Some(inc) = our_inc {
            let inc_share = inc / INC_FACTOR.1 * INC_FACTOR.0;
            budget = budget.saturating_add(inc_share);
        }

        // use a slice of the total remaining time.
        if let Some(t) = our_time {
            budget = budget.saturating_add(t / est_moves_left);
        }

        if let (Some(inc_1), Some(inc_2)) = (our_inc, their_inc) {
            // if there's a difference in increments,
            // we should be more cautious because the opponent
            // will have more time later in the game
            match inc_1.cmp(&inc_2) {
                Ordering::Greater => {
                    // we have an advantage, so we can use a little bit more time
                    budget = budget.saturating_add((inc_1 - inc_2) / est_moves_left);
                }
                Ordering::Less => {
                    // opponent has the advantage, use a little less time in this case
                    budget = budget.saturating_sub((inc_2 - inc_1) / est_moves_left);
                }
                Ordering::Equal => {
                    // do nothing, the timing is fair :)
                }
            }
        }

        if let (Some(t1), Some(t2)) = (our_time, their_time) {
            // we can take a hint from our opponent, and play more naturally
            // closer to their pace. if the opponent is playing
            // quickly, we should speed up (as is more natural in human games)
            match t1.cmp(&t2) {
                Ordering::Less => {
                    // maybe we should think faster
                    let diff = t2 - t1;
                    if diff > CONCERN_THRESHOLD / 2 {
                        budget = budget.saturating_sub(diff / est_moves_left);
                    }
                }
                Ordering::Equal => {
                    // the game is flowing evenly :)
                    budget = budget.saturating_sub(SUBMIT_DURATION);
                    // tiny change to get tiny advantage.
                }
                Ordering::Greater => {
                    // we seem to have more time left than the opponent.
                    // is this difference significant?
                    let diff = t1 - t2;
                    if diff > CONCERN_THRESHOLD {
                        // maybe we should think longer about our moves
                        budget = budget.saturating_add(diff / (est_moves_left / 4).max(1));
                    }
                }
            }
        }

        // always account for submit/IO overhead, and never go below MIN_THINK.
        let mut deadline = if let Some(d) =
            now.checked_add(budget.saturating_sub(SUBMIT_DURATION).max(MIN_THINK))
        {
            d
        } else {
            max_instant()
        };

        // hard cap by absolute remaining time, if present.
        if let Some(t) = our_time
            && let Some(cap) = now.checked_add(t.saturating_sub(SUBMIT_DURATION).max(MIN_THINK))
            && deadline > cap
        {
            deadline = cap;
        }

        // if completely unconstrained, choose to ponder or set a generous cap.
        if our_time.is_none() && our_inc.is_none() {
            deadline = now.checked_add(MAX_THINK).unwrap_or_else(max_instant);
        }

        if let Some(rem) = deadline.checked_duration_since(now)
            && let Some(t) = our_time
        {
            debug_assert!(rem <= t.saturating_sub(SUBMIT_DURATION).max(MIN_THINK));
        }

        self.set_search_until(deadline)
    }
}
